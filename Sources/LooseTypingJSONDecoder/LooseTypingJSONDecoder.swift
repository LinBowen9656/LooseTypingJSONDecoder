//
//  LooseTypingJSONDecoder.swift
//
//
//  Created by BowenLin on 2022/8/29.
//
...
/// `LooseTypingJSONDecoder` facilitates the decoding of JSON into semantic `Decodable` types.
open class LooseTypingJSONDecoder {
    // MARK: Options
    ...
    open func decode<T: Decodable>(_ type: T.Type, from data: Data) throws -> T {
        do {
            ...
            return try JSONDecoderImpl(userInfo: userInfo, from: json, codingPath: [], options: options).unwrap(as: T.self)
...
fileprivate struct JSONDecoderImpl {
    ...
    let json: JSONValue
    let options: LooseTypingJSONDecoder._Options
    
    init(userInfo: [CodingUserInfoKey: Any], from json: JSONValue, codingPath: [CodingKey], options: LooseTypingJSONDecoder._Options) {
...
extension JSONDecoderImpl: Decoder {
    ...
    func unwrap<T: Decodable>(as type: T.Type) throws -> T {
        ...
        if T.self is _JSONStringDictionaryDecodableMarker.Type {
            return try unwrapDictionary(as: T.self)
        }
    ...
    private func unwrapDecimal() throws -> Decimal {
        guard case .number(let numberString) = json else {
    ...
    private func unwrapDictionary<T: Decodable>(as: T.Type) throws -> T {
        ...
        for (key, value) in object {
            ...
            let newDecoder = JSONDecoderImpl(userInfo: userInfo, from: value, codingPath: newPath, options: options)
        ...
        as type: T.Type) throws -> T
    {
            ...
            options.nonConformingFloatDecodingStrategy
        {
            if string == posInfString {
                ...
                return T.nan
            } else if let floatingPoint = T(string) {
                return floatingPoint
            }
        }
...
extension JSONDecoderImpl {
    struct SingleValueContainer: SingleValueDecodingContainer {
        ...
        func decode(_: Bool.Type) throws -> Bool {
            switch value {
            case .string(let string):
                if ["y", "yes", "true"].contains(string.lowercased()) {
                    return true
                } else if ["n", "no", "false"].contains(string.lowercased()) {
                    return false
                } else {
                    guard let value = NSNumber.fromJSONNumber(string) else {
        ...
        init(impl: JSONDecoderImpl, codingPath: [CodingKey], dictionary: [String: JSONValue]) {
            ...
            case .convertFromSnakeCase:
                ...
                dictionary.forEach { (key, value) in
                    converted[LooseTypingJSONDecoder.KeyDecodingStrategy._convertFromSnakeCase(key)] = value
                }
