        ...
        private mutating func readUTF8StringTillNextUnescapedQuote() throws -> String {
            ...
            while let byte = peek(offset: copy) {
                ...
                case 0 ... 31:
                    ...
                    string += makeStringFast(array[stringStartIndex ... errorIndex])
                    throw JSONError.unescapedControlCharacterInString(ascii: byte, in: string, index: errorIndex)

                case UInt8(ascii: "\\"):
                    moveReaderIndex(forwardBy: copy)
                    if output != nil {
                        output! += makeStringFast(array[stringStartIndex ..< stringStartIndex + copy])
                    } else {
                        output = makeStringFast(array[stringStartIndex ..< stringStartIndex + copy])
                    }
                    ...
                    do {
                        ...
                        output! += makeStringFast(array[escapedStartIndex ..< readerIndex])
        ...
        private mutating func parseNumber() throws -> String {
            ...
            while let byte = peek(offset: numberchars) {
                ...
                case ._space, ._return, ._newline, ._tab, ._comma, ._closebracket, ._closebrace:
                    ...
                    return makeStringFast(self[numberStartIndex ..< readerIndex])
                default:
                    throw JSONError.unexpectedCharacter(ascii: byte, characterIndex: readerIndex + numberchars)
                }
            }
            ...
            defer { readerIndex = array.endIndex }
